...
8、set_split方法通过self._lookup_dict选择对应的features和 targets。
...
11、模型输出层fc1的out_features等于hidden_dim参数的值。

## 训练流程
12、generate_batches函数通过PyTorch的DataLoader类实现批量加载。

13、训练时classifier.train()的作用是启用训练和 dropout模式。

14、反向传播前必须执行optimizer.zero_grad()清空梯度。

15、compute_accuracy中y_pred_indices通过torch.argmax()方法获取预测类别。
